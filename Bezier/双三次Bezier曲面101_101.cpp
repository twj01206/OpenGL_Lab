...
void display(void)
{
	...
	//也可选水平方向
	//当然也可以设置  u的变化为0.1 此时就是11个点  数组设置为points_u[11][4][3]   最后有 11*11  
	double points_u[101][4][3] = { 0.0 };
	for (int j = 0; j < 4; j++) {
		int t = 0;
		for (double u = 0.0; u <= 1.0; u += 0.01)
		{
			double B0u = (1 - u) * (1 - u) * (1 - u);
			double B1u = 3 * (1 - u) * (1 - u) * u;
			double B2u = 3 * (1 - u) * u * u;
			double B3u = u * u * u;
			points_u[t][j][0] = R[0][j][0] * B0u + R[1][j][0] * B1u + R[2][j][0] * B2u + R[3][j][0] * B3u;
			points_u[t][j][1] = R[0][j][1] * B0u + R[1][j][1] * B1u + R[2][j][1] * B2u + R[3][j][1] * B3u;
			points_u[t][j][2] = R[0][j][2] * B0u + R[1][j][2] * B1u + R[2][j][2] * B2u + R[3][j][2] * B3u;
	...
	// 行列 101*101
	double points_uv[101][101][3] = { 0.0 };
	for (int j = 0; j < 101; j++) {
		int i = 0;
		for (double t = 0.0; t <= 1.0; t += 0.01)
		{
			double a1 = (1 - t) * (1 - t) * (1 - t);
			double a2 = 3 * (1 - t) * (1 - t) * t;
			double a3 = 3 * t * t * (1 - t);
			double a4 = t * t * t;
	...
	for (int i = 0; i <101; i++) {
		glBegin(GL_LINE_STRIP);
		for (int j = 0; j < 101; j++)
			glVertex3dv(&points_uv[i][j][0]);
		glEnd();

		glBegin(GL_LINE_STRIP);
		for (int j = 0; j < 101; j++)
			glVertex3dv(&points_uv[j][i][0]);
		glEnd();
	}
